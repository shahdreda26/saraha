...
export const signupSchema= {
    body: joi.object({
        userName : joi.string().required(),//.min(5).max(20),//if min(5)اقل عدد حروف  & max(5)اكبر عدد حروف  --> length(5)عدد الحروف لازم يبقى 5
        ...
        email:joi.string().required().email(),// email() --> shahd@gmail.com  
        ...
        password:joi.string().required(),//.length(8).messages({
        ...
        // }),
        cpassword:joi.string().required(),//valid(joi.ref("password"))-->بحيث تبقى زيها password  فكانى بقوله شاور على قيمه ال password هى هى قيمه cpassword يعنى خلى قيمه ال 
        gender:joi.string().required(),//.valid(genderEnum.female,genderEnum.male),
        //عشان اتعامل مع ال enums : valid(genderEnum.female,genderEnum.male)  or valid(...Object.values(genderEnum))
        age:joi.number().required(),//.min(18).max(60),//اقل سن يبقى 18 واكبر سن يبقى 60 - less(40) العمر لازم يكون اقل من 40
    ...
    .required().messages({//بمعنى اى حاجه متعرفهاش عديها options({allowUnknown:true}) احط required() ممكن بدل ال 
    ...
    })//.with("password","cpassword") required معناها ان لازم الاتنين يجوا مع بعض حتى لو مش  & or("password","cpassword") معناها ابعت دا او دا او الاتنين لاكن لازم يبعت واحد فيهم على الاقل 
